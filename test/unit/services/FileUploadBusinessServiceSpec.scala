...
class FileUploadBusinessServiceSpec extends UnitSpec with MockitoSugar {
  ...
  trait SetUp {
    ...
    val upscanInitiatePayload = UpscanInitiatePayload("http://file-upload-upscan-callback.url/uploaded-file-upscan-notifications/clientSubscriptionId/327d9145-4965-4d28-a2c5-39dedee50334", 10000, "https://success-redirect.com", "https://error-redirect.com")
  ...
  "FileUploadBusinessService" should {
    "send payload to connector for non-CSP" in new SetUp() {
      ...
      verify(mockUpscanInitiateConnector, atLeastOnce()).send(meq(upscanInitiatePayload), meq(VersionOne))(meq(jsonRequest))
    }
    ...
    "send payload to connector for CSP" in new SetUp() {
      ...
      result shouldBe xmlResponse
      verify(mockUpscanInitiateConnector, atLeastOnce()).send(meq(upscanInitiatePayload), meq(VersionOne))(meq(ValidatedFileUploadPayloadRequestForCspWithTwoFiles))
    }
