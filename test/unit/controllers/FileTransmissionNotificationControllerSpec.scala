...
import uk.gov.hmrc.customs.file.upload.controllers.FileTransmissionNotificationController
import uk.gov.hmrc.customs.file.upload.model.FileTransmissionNotification
...
class FileTransmissionNotificationControllerSpec extends PlaySpec
  ...
  "file transmission notification controller" should {
    ...
    "return 500 when call to Custom Notification services fails" in new SetUp {
      when(mockService.sendMessage[FileTransmissionNotification](SuccessNotification, SuccessNotification.fileReference, subscriptionFieldsId)(callbackToXmlNotification)).thenReturn(Future.failed(emulatedServiceFailure))
