...
import uk.gov.hmrc.customs.file.upload.controllers.CustomHeaderNames
import uk.gov.hmrc.customs.file.upload.logging.LoggingHelper
...
class LoggingHelperSpec extends UnitSpec with MockitoSugar {
  ...
  "LoggingHelper" should {
    ...
    "testFormatInfo with authorisation" in {
      LoggingHelper.formatInfo("Info message", validatedHeadersRequest.toCspAuthorisedRequest(Csp(badgeIdentifier))) shouldBe expectedMessage("Info message", "[authorisedAs=Csp(BADGEID123)]")
    }
