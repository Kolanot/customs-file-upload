...
import uk.gov.hmrc.customs.file.upload.model.actionbuilders.{AuthorisedRequest, ConversationIdRequest}
import uk.gov.hmrc.play.test.UnitSpec
import util.MockitoPassByNameHelper.PassByNameVerifier
import util.TestData._

class FileUploadLoggerSpec extends UnitSpec with MockitoSugar {

  trait SetUp {
    val mockCdsLogger: CdsLogger = mock[CdsLogger]
    val logger = new FileUploadLogger(mockCdsLogger)
    implicit val implicitVpr: AuthorisedRequest[AnyContentAsXml] = ConversationIdRequest(conversationId, request = FakeRequest()
